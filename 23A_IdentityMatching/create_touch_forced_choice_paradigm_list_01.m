function [ output_args ] = create_touch_forced_choice_paradigm_list_01( input_args )
...
attribute_set = 'ID'; % ID, view, all

touchforcedchoice_xml_image_section = ['touchforcedchoice_xml_image_section_', attribute_set,'_attributes_01.txt'];	% the name of the image list

% where to collect the images from relative to this script
relative_target_img_dir = fullfile('..' , 'AM_familiarity_02');	% where to find the potential target images, relative to this script's directory

% what should end up in the xml file, ATTENTION windows pathnames
% recommended
kofiko_system = 'KOFIKO-USER-23A';
abs_img_path_4_kofiko = ['\\', kofiko_system, '\', 'StimulusSet', '\', 'AM_familiarity_02'];
main_exp_name_string = 'ID_match_v01';	% just a convenient way to include an arbitrary attribute for the whole list


%ID_selection = {'MS', 'SN', 'ME', 'OS', 'maurice', 'bert', 'feivel', 'julien'};
ID_selection = {'MS', 'SN', 'ME', 'OS'};	% this affects both targets and same cat distractors
ID_selection = {};
...
for i_img = 1 : n_same_cat_imgs
	...
	Image_Name_list{i_img} = ['ID-', img_properties{cur_img_idx}.ID, '_ORI-A', num2str(img_properties{cur_img_idx}.AZI, '%03d')];
	
	switch attribute_set
		case 'all'
			% full attribute list
			Attr_list{i_img} = [main_exp_name_string, ';', ['ID_', img_properties{cur_img_idx}.ID], ';', ['AZI_', num2str(img_properties{cur_img_idx}.AZI, '%03d')], ';', TYPE_list{i_img}, ';', FAMILIARITY_list{i_img}, ';', SOURCE_list{i_img}];
		case 'ID'
			% ID attribute list
...
% now write the choice lines to the file
