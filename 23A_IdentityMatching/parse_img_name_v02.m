	...
	case 'A'
		...
		if ~isempty(ele_descriptor_idx);
			...
			if ~isnan(proto_azi)
				...
				% now get ele_end_idx
				ele_start_idx = ele_descriptor_idx(1) + 1;
				proto_ele = str2double(cur_comp_data_string(ele_start_idx : end));
				if ~isnan(proto_ele)
	...
	case 'E'
		...
		if ~isempty(azi_descriptor_idx);
			...
			if ~isnan(proto_ele)
				prop_struct_attr_ORI.ele = proto_ele;
				% now get azi_end_idx
				azi_start_idx = azi_descriptor_idx(1) + 1;
				proto_azi = str2double(cur_comp_data_string(azi_start_idx : end));
				if ~isnan(proto_azi)
					prop_struct_attr_ORI.azi = proto_azi;
				else
					%2nd ORI string type, A000-001_A00_E00
