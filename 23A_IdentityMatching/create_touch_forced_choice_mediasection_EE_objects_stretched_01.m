function [ output_args ] = create_touch_forced_choice_mediasection_EE_objects_stretched_01( input_args )
...
transform_type = 'stretched';	% masked or											stretched
transform_instance = 'stretched'; % randomdot, randomdot_lp, phase_scramble;	stretched
...
relative_target_img_dir = fullfile('..' , ['AM_familiar_objects_CIT_01_', transform_type], transform_instance);	% where to find the potential target images, relative to this script's directory

% what should end up in the xml file, ATTENTION windows pathnames
% recommended
kofiko_system = 'KOFIKO-USER-23A';
abs_img_path_4_kofiko = ['\\', kofiko_system, '\', 'StimulusSet', '\', ['AM_familiar_objects_CIT_01_', transform_type], '\', transform_instance];
...
AZI_string_start = strfind(ORI_string(1:end), '_A');
AZI_string_end = min([(strfind(ORI_string(AZI_string_start:end), '_E') - 2 + AZI_string_start) strfind(ORI_string, '.') - 1]);
ELE_string_start = strfind(ORI_string(1:end), '_E');
ELE_string_end = min([(strfind(ORI_string(ELE_string_start + 4:end), '-') - 2 + ELE_string_start + 4) strfind(ORI_string, '.') - 1]);
short_ID_AZI_string = ORI_string(short_ID_AZI_string_start(1) - 1: strfind(ORI_string, '.') - 1);
field_sep_idx = strfind(short_ID_AZI_string, '-');

prop_struct.ID_AZI_ELE = short_ID_AZI_string;
prop_struct.ID = short_ID_AZI_string(field_sep_idx(end) + 1: end);
if (length(field_sep_idx) > 1)
	prop_struct.AZI = str2num(ORI_string(AZI_string_start + 2: AZI_string_end));
	prop_struct.ELE = str2num(ORI_string(ELE_string_start + 2: ELE_string_end));
else
...
ID_end_idx = max([ strfind(img_name_string, '_CIT_familiar') ...
	...
	strfind(img_name_string, '_CIT_EE_objects') ...
...
elseif ~isempty(strfind(img_name_string, '_CIT_EE_objects'))
	prop_struct.SOURCE = 'CIT';
	prop_struct.FAMILIARITY = 'familiar';
