...
attribute_set = 'ID-AZI'; % ID, ID-TF, view, all
subset = 'objects_';
...
abs_img_path_4_kofiko = ['\\', kofiko_system, '\', 'StimulusSet', '\', ['AM_familiar_objects_CIT_01']];
...
[img_list, img_properties] = get_names_and_props_from_dir_and_wildcard(fullfile(cur_mfiledir, relative_target_img_dir), '*.png');
...
for i_img = 1 : n_same_cat_imgs
	...
	FAMILIARITY_list{i_img} = img_properties{cur_img_idx}.attr.ID.FAMILIARITY;
 	TRANSFORMATION_list{i_img} = 'none';%img_properties{cur_img_idx}.attr.TRF.transform.descriptor;
	...
	Image_Name_list{i_img} = ['ID-', img_properties{cur_img_idx}.attr.ID.identity, '_ORI-A', num2str(img_properties{cur_img_idx}.attr.ORI.azi, '%03d'), '_E', num2str(img_properties{cur_img_idx}.attr.ORI.ele, '%03d'), '_TRF-', TRANSFORMATION_list{i_img}];
...
% only objects
img_selection_list = OBJECT_img_idx;
% default is 5 views
...
for i_matching_file = 1 : n_matching_files
	...
	file_properties{i_matching_file} = parse_img_name_v03(proto_file_list(i_matching_file).name);
end
