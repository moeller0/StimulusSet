function [ output_args ] = create_touch_forced_choice_mediasection_objects_nonstretched_01( input_args )
...
transform_type = '';	% masked or											stretched
transform_instance = ''; % randomdot, randomdot_lp, phase_scramble;	stretched
attribute_set = 'ID'; % ID, ID-TF, view, all
subset = 'objects_';

touchforcedchoice_xml_image_section = ['touchforcedchoice_xml_image_section_', subset, '_', attribute_set,'_attributes_01.txt'];	% the name of the image list

% where to collect the images from relative to this script
relative_target_img_dir = '.';%fullfile('..' , ['AM_familiar_objects_CIT_01']);	% where to find the potential target images, relative to this script's directory

% what should end up in the xml file, ATTENTION windows pathnames
% recommended
kofiko_system = 'KOFIKO-USER-23A';
abs_img_path_4_kofiko = ['\\', kofiko_system, '\', 'StimulusSet', '\', ['abstracted_houses_and_faces_02']];
main_exp_name_string = 'objects';	% just a convenient way to include an arbitrary attribute for the whole list
...
max_IDs = -1;	% if not negative restrict the number of IDs to the minimum of max_IDs and existing IDs
...
[img_list, img_properties] = get_names_and_props_from_dir_and_wildcard(fullfile(cur_mfiledir, relative_target_img_dir), '*.*');
...
for i_img = 1 : n_same_cat_imgs
	...
	%ID_AZI_ELE_list{i_img} = img_properties{cur_img_idx}.attr.ORI.ID_AZI_ELE;
	ID_list{i_img} = img_properties{cur_img_idx}.attr.ID.identity;
	AZI_list(i_img) = img_properties{cur_img_idx}.attr.ORI.azi;
	%ELE_list(i_img) = img_properties{cur_img_idx}.attr.ORI.ele;
	SOURCE_list{i_img} = img_properties{cur_img_idx}.attr.ID.SOURCE;
	TYPE_list{i_img} = img_properties{cur_img_idx}.attr.ID.TYPE;
	FAMILIARITY_list{i_img} = img_properties{cur_img_idx}.attr.ID.FAMILIARITY;
 	TRANSFORMATION_list{i_img} = img_properties{cur_img_idx}.attr.TRF.string;
	...
	Image_Name_list{i_img} = ['ID_', img_properties{cur_img_idx}.attr.ID.identity, '-ORI_A', num2str(img_properties{cur_img_idx}.attr.ORI.azi, '%03d'), '_E', num2str(img_properties{cur_img_idx}.attr.ORI.ele, '%03d'), '-TRF_', TRANSFORMATION_list{i_img}];
	
	switch attribute_set
		case 'all'
			% full attribute list
			Attr_list{i_img} = [main_exp_name_string, ';', ['ID_', img_properties{cur_img_idx}.attr.ID.identity], ';', ['AZI_', num2str(img_properties{cur_img_idx}.attr.ORI.azi, '%03d')], ';', TYPE_list{i_img}, ';', FAMILIARITY_list{i_img}, ';', SOURCE_list{i_img}, ';', TRANSFORMATION_list{i_img}];
		case 'ID-AZI'
			% full attribute list
			Attr_list{i_img} = [main_exp_name_string, ';', ['ID_', img_properties{cur_img_idx}.attr.ID.identity], ';', ['AZI_', num2str(img_properties{cur_img_idx}.attr.ORI.azi, '%03d')]];
		case 'ID'
			% ID attribute list
			Attr_list{i_img} = [main_exp_name_string, ';', ['ID_', img_properties{cur_img_idx}.attr.ID.identity]];
		case 'ID-TF'
			% ID attribute list
			Attr_list{i_img} = [main_exp_name_string, ';', ['ID_', img_properties{cur_img_idx}.attr.ID.identity], ';', TRANSFORMATION_list{i_img}];

		case 'view'
			% view attribute list
			Attr_list{i_img} = [main_exp_name_string, ';', ['AZI_', num2str(img_properties{cur_img_idx}.attr.ORI.azi, '%03d')]];
		otherwise
...
% only objects
%img_selection_list = HUMAN_img_idx;
% default is 5 views
% % frontal plus half-profiles
% img_selection_list = setdiff(img_selection_list, PROFILE_idx);
% % frontal only
% img_selection_list = intersect(img_selection_list, FRONTAL_idx);	%
...
% clean-up
fclose(fcd_fh);
disp(['output file: ', fullfile(cur_mfiledir, touchforcedchoice_xml_image_section)]);
...
% 20130215sm: modified to deal with multiple image file types...
...
for i_matching_file = 1 : n_matching_files
	[cur_path, cur_name, cur_ext] = fileparts(proto_file_list(i_matching_file).name);
	switch cur_ext
		case {'.jpg', '.tif', '.png', '.bmp', '.jpeg', '.tiff', '.JPG', '.TIF', '.PNG', '.BMP', '.JPEG', '.TIFF'}
			...
			file_properties{i_matching_file} = parse_img_name_v07(proto_file_list(i_matching_file).name);
		otherwise
			bad_file_idx(end+1) = i_matching_file;	
	end
end

file_list(bad_file_idx) = [];
file_properties(bad_file_idx) = [];
