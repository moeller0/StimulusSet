function [prop_struct] = parse_img_name_v07(img_name_string)
...
% 20121228sm: changed to correctly parse the cat names
...
for i_comp = 1 : length(sorted_component_string_list)
	...
	switch cur_comp_type
		...
		case 'EXP'
			% expression
			prop_struct.attr.(cur_comp_type) = parse_EXP_string(cur_comp_data_string, img_name_string);
			prop_struct.attr.(cur_comp_type).string = cur_comp_data_string;
		case 'TRF'
...
if isfield(prop_struct.attr, 'TRF')
	...
	% create a name for the current transform, by concatenating all individual
	% transforms performed on the image (typically there should only be one transform, but in the spirit of keeping it generic... (awaits testing))
	% find all transforms...
	prop_struct.attr.TRF.name = concatenate_all_transform_names(prop_struct);
	prop_struct.attr.TRF.([prop_struct.attr.TRF.name, '_pct']) = 100;	% TODO fill with a meaningful value
end
...
% allow transformation names without value, like mooney, linedrawing
if ~isempty(sep_idx)
	trf_type = prop_struct_attr_TRF.string(1 : sep_idx(1) - 1);
else
	trf_type = prop_struct_attr_TRF.string;
end

prop_struct_attr_TRF.(trf_type).name = trf_type;	% the name of the current transform
% allow transformation names with out value, like mooney, linedrawing
if ~isempty(sep_idx)
	trf_value_string = cur_comp_data_string(sep_idx(1) : end);
else
	trf_value_string = '_100pct';	%assume that qualitative transforms like silhouette, line drawing and such are just extremes of a sparse continuum
end
unit_suffix = '';
...
switch trf_type
	case {'mooney', 'cartoon', 'linedrawing', 'silhouette', 'cartton2', 'mooney2', 'mooney_ud', 'mooney2_ud'}
		prop_struct_attr_TRF.string;
		% full individual transform structure
		prop_struct_attr_TRF.(trf_type).name = trf_type;
...
ID_end_idx = max([ strfind(cur_comp_data_string, '_CIT_familiar') ...
	...
	strfind(cur_comp_data_string, 'tweened_')...
	strfind(cur_comp_data_string, '_ALOI_')...
	strfind(cur_comp_data_string, '_CIT_2D')...
	strfind(cur_comp_data_string, '_ALOI')...
	strfind(cur_comp_data_string, '_CIT_houses')...
	]) - 1;
...
if (~isempty(strfind(cur_comp_data_string, 'felis_catus')))
	prop_struct_attr_ID.TYPE = 'cat';
	prop_struct_attr_ID.SOURCE = 'WWW';
	prop_struct_attr_ID.FAMILIARITY = 'nonfamiliar';
end
if (~isempty(strfind(cur_comp_data_string, 'objects-house')))
	prop_struct_attr_ID.TYPE = 'house';
	prop_struct_attr_ID.SOURCE = 'WWW';
	prop_struct_attr_ID.FAMILIARITY = 'nonfamiliar';
end

prop_struct_attr_ID.SOURCE = '';
prop_struct_attr_ID.FAMILIARITY = '';
...
elseif ~isempty(strfind(cur_comp_data_string, '_MultiPIE'))
	...
	prop_struct_attr_ID.FAMILIARITY = 'nonfamiliar';
elseif ~isempty(strfind(cur_comp_data_string, '_ALOI'))
	prop_struct_attr_ID.SOURCE = 'ALOI';
	prop_struct_attr_ID.FAMILIARITY = 'nonfamiliar';
elseif ~isempty(strfind(cur_comp_data_string, '_CIT_2D'))
	prop_struct_attr_ID.SOURCE = 'CIT';
	prop_struct_attr_ID.FAMILIARITY = 'nonfamiliar';
end
...
% examples: A-45_E010, A000-001_A00_E00, A000-001_01_01_051_07 (PIE has weird position codes)
prop_struct_attr_ORI.azi = 99999999;
prop_struct_attr_ORI.ele = 99999999;
extract_ori_harder = 0;
extract_ori_harder2 = 0;
...
if (extract_ori_harder2)
	% we either have only AZI or only ELE
	azi_start_idx = strfind(cur_comp_data_string, 'A');
	ele_start_idx = strfind(cur_comp_data_string, 'E');
	if ~isempty(ele_start_idx)
		% just leave AZI set to empty marker
		if ~isempty(strfind(cur_comp_data_string(3:end), '-'))
			proto_ele_end_idx = strfind(cur_comp_data_string(3:end), '-');
			ele_end_idx = proto_ele_end_idx + 1;
			proto_ele = str2double(cur_comp_data_string(ele_start_idx + 1 : ele_end_idx));
			if ~isnan(proto_ele)
				prop_struct_attr_ORI.ele = proto_ele;
			end
		end
		...
		% just leave ELE set to empty marker
		if ~isempty(strfind(cur_comp_data_string(3:end), '-'))
			proto_azi_end_idx = strfind(cur_comp_data_string(3:end), '-');
			azi_end_idx = proto_azi_end_idx + 1;
			proto_azi = str2double(cur_comp_data_string(azi_start_idx + 1 : azi_end_idx));
			if ~isnan(proto_azi)
				prop_struct_attr_ORI.azi = proto_azi;
			end
		end
...
function [ prop_struct_attr_EXP ] = parse_EXP_string(cur_comp_data_string, img_name_string)
% examples: EXP_50.0pct_Anger, or EXP_Anger

prop_struct_attr_EXP.string = cur_comp_data_string;	% was .descriptor
% split by _
sep_idx = strfind(cur_comp_data_string, '_');
% magnitude of expression
if ~isempty(sep_idx) & ~isempty(str2num(cur_comp_data_string(1)))
	prop_struct_attr_EXP.expression = cur_comp_data_string(sep_idx + 1 : end);
	unit_idx = strfind(cur_comp_data_string, 'pct');	% so far we only have percent...
	if ~isempty(unit_idx),
		unit_suffix = '_pct';
		prop_struct_attr_EXP.(['magnitude', unit_suffix]) = str2num(cur_comp_data_string(1: unit_idx - 1));
	end
else
	% only expression name...
	prop_struct_attr_EXP.expression = cur_comp_data_string;
end
...
function [ long_name ] = concatenate_all_transform_names(cur_prop_struct)
% find all sub transforms and concatenate their names to get an robust
% idetifier of the possibly chained transformations

proto_transformname_list = fieldnames(cur_prop_struct.attr.TRF);
long_name = [];

for i_field = 1 : length(proto_transformname_list)
	% skip the "string" fields
	if ~strcmp(proto_transformname_list{i_field}, 'string')
		%cur_value = cur_prop_struct.attr.TRF.(proto_transformname_list{i_field}).([proto_transformname_list{i_field}, '_pct');
		if isempty(long_name)
			long_name = proto_transformname_list{i_field};
		else
			long_name = [long_name, '_', proto_transformname_list{i_field}];
		end
	end
end
